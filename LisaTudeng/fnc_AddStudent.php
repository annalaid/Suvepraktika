...
//funktsioonid, mida või kuidas kasutada
  $database = "if20_pille_suvepraktika";
    ...
    function addstudent($firstname, $lastname, $email, $personal_email){
      echo "algus";
      ...
      if($stmt->execute()) {
        if($stmt->fetch()) {
          ...
          $stmt->close();
          $stmt = $conn->prepare("INSERT INTO student (firstname, lastname, email, personal_email) VALUES (?, ?, ?, ?)");
          echo $conn->error;
          $stmt->bind_param("ssss", $firstname, $lastname, $email, $personal_email);
          if($stmt->execute()) {
