	...
	if(isset($_POST["personsubmit"])) {
		...
		if(!empty($_POST["emailinput"])){
          ...
          if(!empty($_POST["tag_nameinput"])){
              $chosedtag = intval($_POST["tag_nameinput"]);
          } else {
              $taginputerror .= " Vali silt !";

          }

		if(empty($firstnameerror) and empty($lastnameerror) and empty($emailerror) and empty($personal_emailerror) and empty($uliopilaskooderror) and empty($taginputerror)) {
			$studentnotice = addstudent($firstname, $lastname, $email, $personal_email, $uliopilaskood, $_POST["tag_nameinput"]);
		}
